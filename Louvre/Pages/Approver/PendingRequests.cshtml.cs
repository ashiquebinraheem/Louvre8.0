...
namespace Louvre.Pages
{
    ...
    public class PendingRequestsModel : BasePageModel
    {
        private readonly IDbContext _dbContext;

        public PendingRequestsModel(IDbContext entity)
        {
            _dbContext = entity;
        }
        ...
        LEFT JOIN RequestType T ON R.RequestTypeID = T.RequestTypeID";
            ...
            // Base condition (safe because enums)
            SearchData.WhereCondition = $@"
        StatusID IN ({(int)RequestStatus.Pending}, {(int)RequestStatus.Processing})";

            // Role-based filter
            if (User.IsInRole("Meterial"))
            {
                // Force int conversion to avoid injection
                SearchData.WhereCondition += $" AND RequestedByID = {Convert.ToInt32(CurrentUserID)}";
            }
            else
            {
                // Force int conversion for UserTypeID as well
                SearchData.WhereCondition += $@"
