...
namespace Louvre.Pages.DailyPass
{
    ...
    [BindProperties]
    public class DailyPassRequestModel : BasePageModel
    {
        ...
        public DailyPassRequestModel(IDbContext dbContext, IDbConnection cn, ICommonRepository commonRepository, IMediaRepository mediaRepository, IHttpContextAccessor httpContextAccessor, IErrorLogRepository errorLogRepo)
        {
        ...
        public DailyPassRequest Data { get; set; }
        ...
        public async Task OnGetAsync(int? id)
        {
            ...
            ViewData["RequestModes"] = (await _dbContext.GetAllAsync<RequestMode>()).ToList().Where(s => s.IsIn == true);
            ViewData["Locations"] = await _dbContext.GetAllAsync<Location>();
            ViewData["MeterialTypes"] = new SelectList((await _dbContext.GetAllAsync<RequestMeterialType>()).ToList().Select(s => new IdnValuePair() { ID = Convert.ToInt32(s.MeterialTypeID), Value = s.MeterialTypeName }), "ID", "Value");
            if (id != null)
            {
                Data = await _dbContext.GetAsync<DailyPassRequest>(Convert.ToInt32(id));
            ...
            if (Data != null)
            {
                ...
                Passengers = (await _dbContext.GetAllAsyncByFieldName<RequestPassenger>("DailyPassRequestID", id.ToString())).ToList();
                ...
                RequestDocuments = await _commonRepository.GetDocumentsAsync(DocumentTypeCategory.DailyPassRequest, Convert.ToInt32(Data.DailyPassRequestID));
                RequestStatusID = await _dbContext.ExecuteScalarAsync<int>($"Select StatusID From viDailyPassRequest where DailyPassRequestID={Data.DailyPassRequestID}", null);
                if (RequestStatusID == (int)RequestStatus.Rejected)
                {
                    RequestApproval = await _dbContext.GetAsync<RequestApprovalViewModel>($@"SELECT DailyPassRequestID, LocationID,StorageLocationID, StatusID, Remarks
            ...
            {
                RequestDocuments = await _commonRepository.GetDocumentsAsync(DocumentTypeCategory.DailyPassRequest, 0);
            }
        ...
        public async Task<IActionResult> OnPostSaveAsync()
        {
            ...
            var rateLimitCnt = await _dbContext.GetAsync<int>($@"Select Count(*) From DailyPassRequest 
            ...
            using (var tran = cn.BeginTransaction())
            {
                try
                {
                    ...
                    if (Data.RequestNo == 0)
                    {
                        Data.RequestNo = await _dbContext.GetAsync<int>("Select ISNULL(Max(RequestNo),0)+1 from DailyPassRequest", null, tran);
                    }
                    Data.PassengersCount = Passengers.Count();
                    var requestId = await _dbContext.SaveAsync(Data, tran);

                    await _dbContext.SaveSubListAsync(Meterials, "DailyPassRequestID", requestId, tran);

                    await _dbContext.SaveSubListAsync(Passengers, "DailyPassRequestID", requestId, tran);

                    #region Meterial Documents


                    var meterialDocuments = new List<RequestMeterialMedia>();

                    for (int i = 0; i < MeterialMedias.Count; i++)
                    {
                        var mediaResult = await _mediaRepository.SaveMedia(null, MeterialMedias[i], "meterial_documents", requestId + "_" + i, tran);
                        if (mediaResult.IsSuccess)
                        {
                            var document = new RequestMeterialMedia
                            {
                                MediaID = mediaResult.MediaID,
                            };

                            meterialDocuments.Add(document);
                        }

                    }
                    await _dbContext.SaveSubListAsync(meterialDocuments, "DailyPassRequestID", requestId, tran);
                    ...
                    await _dbContext.SaveSubListAsync(documents, "DailyPassRequestID", requestId, tran);

                    #endregion

                    await _dbContext.DeleteSubItemsAsync<RequestApproval>("DailyPassRequestID", requestId, tran);

                    #region Mail to appprovers

                    var url = $"{this.Request.Scheme}://{this.Request.Host}{this.Request.PathBase}";
                    await _commonRepository.SendNewDailyPassRequestMailToApprover(requestId, url, tran);
        ...
        public async Task<IActionResult> OnPostSaveEmployeeAsync()
        {
            ...
            using (var tran = cn.BeginTransaction())
            {
                ...
                {
                    tran.Rollback();
                    await _errorLogRepo.CreatThrowResponse(err.Message, CurrentUserID);
                    result.CreatThrowResponse(err.Message);
                }
            ...
            using (var tran = cn.BeginTransaction())
            {
                try
                {
                    ...
                    result.Vehicles = (await _dbContext.GetEnumerableAsync<ViVehicle>($@"Select VehicleID, RegisterNo, VehicleTypeName, VehicleSize, PlateNo, VehicleMakeName, 
                        ...
                        From viVehicle Where AddedBy=@AddedBy", new { AddedBy = CurrentUserID })).ToList();
