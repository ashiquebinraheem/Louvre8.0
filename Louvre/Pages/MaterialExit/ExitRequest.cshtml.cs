...
namespace Louvre.Pages
{
    ...
    [BindProperties]
    public class ExitRequestModel : BasePageModel
    {
        ...
        public ExitRequestModel(IDbContext dbContext, IDbConnection cn, ICommonRepository commonRepository, IMediaRepository mediaRepository, IHttpContextAccessor httpContextAccessor, IErrorLogRepository errorLogRepo)
        {
        ...
        public async Task OnGetAsync(int? id)
        {
            ...
            ViewData["Branches"] = await _dbContext.GetAllAsync<Branch>();/*).ToList().Where(s => s.BranchName == "Louvre Abu Dhabi LLC");*/
			...
			ViewData["StorageLocationTypes"] = new SelectList(
				(await _dbContext.GetAllAsync<Location>())
					.Where(a => a.LocationTypeID == 3)
					.Select(s => new IdnValuePair
					{
						ID = Convert.ToInt32(s.LocationID),
        ...
        public async Task<IActionResult> OnPostSaveAsync()
        {
            ...
            //    FROM  Users
            //    Where ISNULL(IsDeleted,0)=0 and UserTypeID not in ({(int)UserTypes.Company},{(int)UserTypes.Individual}) and (EmailAddress=@EmailAddress)", new { EmailAddress = Data.HostEmail });
