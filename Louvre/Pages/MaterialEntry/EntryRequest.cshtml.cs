...
namespace Louvre.Pages
{
    ...
    [BindProperties]
    public class EntryRequestModel : BasePageModel
    {
        ...
        public EntryRequestModel(IDbContext dbContext, IDbConnection cn, ICommonRepository commonRepository, IMediaRepository mediaRepository, IHttpContextAccessor httpContextAccessor, IErrorLogRepository errorLogRepo)
        {
        ...
        public async Task OnGetAsync(int? id)
        {
            ...
            ViewData["RequestModesConstruction"] = (await _dbContext.GetAllAsync<RequestMode>()).ToList().Where(s => s.IsIn == true && s.ModeName == "Construction Access");
            ViewData["Locations"] = await _dbContext.GetAllAsync<Location>();
            ViewData["LocationsConstruction"] = (await _dbContext.GetAllAsync<Location>()).ToList().Where(s => s.LocationName == "Construction Access");
            ...
            if (Data != null)
            {
                ...
                MeterialFiles = (await _dbContext.GetEnumerableAsync<MeterialFileViewModel>($@"Select MM.MeterialMediaID,FileName,RequestID
                    ...
                    Where MM.RequestID=@RequestID and MM.IsDeleted=0", new { RequestID =id})).ToList();
            }
            else
            {
        ...
        public async Task<IActionResult> OnPostSaveAsync()
        {
            ...
            using (var tran = cn.BeginTransaction())
            {
                try
                {
                    ...
                    await _dbContext.SaveSubListAsync(meterialDocuments, "RequestID", requestId, tran);
            ...
            using (var tran = cn.BeginTransaction())
            {
                try
                {
                    ...
                    result.Vehicles = (await _dbContext.GetEnumerableAsync<ViVehicle>($@"Select VehicleID, RegisterNo, VehicleTypeName, VehicleSize, PlateNo, VehicleMakeName, 
                        ...
                        From viVehicle Where AddedBy=@CurrentUserID", new { CurrentUserID = CurrentUserID })).ToList();
