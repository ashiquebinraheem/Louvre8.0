...
namespace Louvre.Pages.ProjectAsset
{
    ...
    public class PendingProjectRequestsModel : BasePageModel
    {
        private readonly IDbContext _dbContext;

        public PendingProjectRequestsModel(IDbContext dbContext)
        {
        ...
        public async Task<IActionResult> OnPostSearchAsync() // Added by Abdul Razack for No Injection Risk
        {
            #region Validation
            ...
            // Safe WhereCondition (force CurrentUserID to int)
            SearchData.WhereCondition = $@"
                ...
                AND ISNULL(IsProjectAsset,0) = 1";

            // Allow-list for SearchColumnName (prevents injection)
        ...
        //    SearchData.WhereCondition = $@"StatusID in ({(int)RequestStatus.Pending},{(int)RequestStatus.Processing},{(int)RequestStatus.Forwarded}) and IsIn=1 and RequestedByID={CurrentUserID} and ISNULL(IsProjectAsset,0)=1";
