...
namespace Louvre.Pages.ProjectAsset
{
    ...
    public class AcceptedProjectRequestsModel : BasePageModel
    {
        private readonly IDbContext _dbContext;

        public AcceptedProjectRequestsModel(IDbContext dbContext)
        {
        ...
        //    SearchData.WhereCondition = $@"StatusID={(int)RequestStatus.Accepted} and IsIn=1 and RequestedByID={CurrentUserID} and ISNULL(IsProjectAsset,0)=1";
        ...
        LEFT JOIN RequestType T ON R.RequestTypeID = T.RequestTypeID";
            ...
            // Sanitize: force CurrentUserID to int, StatusID to enum int
        ...
        AND ISNULL(IsProjectAsset,0) = 1";
            ...
            // Whitelist valid SearchColumnName
            switch (SearchData.SearchColumnName)
            {
                case "Date":
                    SearchData.SearchColumnName = "CONVERT(varchar, RequestedOn,103)";
